...
> copies of the same two. What stays here is `.github/pin-only.yml`, which
> names the pin surfaces, including `images/base/Dockerfile` and the annotated
> `ARG` names read from it.
>
> Renovate bumps the pin, and because changing a pinned `uses:` is itself a
> pin-only diff, that bump merges unattended.
...
An hourly workflow, `coderabbit-review-queue.yml` (`29 * * * *`), used to post
that comment. It was retired on 2026-09-21, on cost rather than on capability:
it posted with `CODERABBIT_NUDGE_TOKEN` so the comment came from a human
account and CodeRabbit answered it within seconds. What it cost is described
two sections down, under what a new repository needed: an organization secret
scoped to a selected list of repositories, which turns any repository left off
that list into a silently failing `gh` call. That is not hypothetical;
`ivan-pinatti-labs/.github` sat in exactly that state, and the job also could
not see the shared review quota it was firing into. See rsync-crypt's
`AGENTS.md`, "CodeRabbit silently ignores `@coderabbitai review` from a bot
account," for why the comment has to come from a human account rather than
from the default `GITHUB_TOKEN`.
